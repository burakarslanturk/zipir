...
function NextGameTimer() {
  ...
  return (
    <div className="mt-8 p-6 bg-violet-50 border border-violet-100 rounded-2xl shadow-sm text-center w-full max-w-sm mx-auto">
      ...
      <div className="text-4xl font-black text-violet-600 font-mono tracking-widest">{timeLeftStr}</div>
    </div>
  );
}
  ...
  if (!hasStarted) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-3">
        <h1 className="text-5xl font-black text-violet-600 mb-6">ZIPIR (Demo)</h1>
        <button onClick={() => setHasStarted(true)} className="px-8 py-4 bg-violet-600 text-white rounded-xl font-bold">Oyuna Başla</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-800">
      <header className="w-full max-w-4xl mx-auto px-4 py-8 relative flex items-center justify-between">
        <div className="flex-1 flex justify-start">
          <div className="text-4xl sm:text-5xl font-black text-violet-500 tracking-tight">ZIPIR <span className="text-xl text-slate-400 ml-2 font-medium">(Demo Layout)</span></div>
        </div>
      </header>

      <main className="flex-1 w-full max-w-4xl mx-auto px-4 flex flex-col justify-center pb-20 pt-4">
        
        {/* KART DIŞI YERLEŞİM: Soru, Puan ve Süre */}
        <div className="w-full flex justify-between items-end mb-3 mt-4 sm:mt-8 px-1 sm:px-0">
            
            <div className="flex gap-3 sm:gap-4">
                <div className="flex flex-col items-start bg-white/60 px-4 py-2.5 rounded-xl shadow-sm backdrop-blur-sm border border-slate-200/50">
                    <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Soru</span>
                    <div className="text-lg font-black text-slate-700">{currentQuestionIndex + 1} <span className="text-slate-400 text-sm font-medium">/ {questions.length}</span></div>
                </div>
            </div>

            <div className="flex gap-3 sm:gap-4">
                <div className="flex flex-col items-start bg-white/60 px-4 py-2.5 rounded-xl shadow-sm backdrop-blur-sm border border-slate-200/50">
                    <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Puan</span>
                    <div className="text-lg font-black text-violet-600">{score}</div>
                </div>

                <div className={`flex flex-col items-end bg-white/60 px-4 py-2.5 rounded-xl shadow-sm backdrop-blur-sm border border-slate-200/50 ${timeLeft <= 30 ? 'ring-2 ring-red-400/50 animate-pulse text-red-500' : 'text-slate-700'}`}>
                    <span className={`text-[11px] font-bold uppercase tracking-widest mb-0.5 ${timeLeft <= 30 ? 'text-red-500' : 'text-slate-400'}`}>Süre</span>
                    <div className="flex items-center">
                        <span className="text-xl font-black font-mono tracking-tight text-right w-[4rem]">{formatTime(timeLeft)}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        {/* ANA KART */}
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-10 flex flex-col items-center relative">
          ...
          <div className={`flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-10 cursor-text ${isShaking || answerStatus === "wrong" ? "animate-shake" : ""}`} onClick={() => isAnswering && document.getElementById('hidden-answer-input')?.focus()}>
            {(() => {
              ...
              return currentQuestion.word.split("").map((letter: string, index: number) => {
                ...
                else if (isRevealed) boxStyle = "bg-violet-50 border-violet-200 text-violet-700 shadow-sm";
                else if (isUserTyped) boxStyle = "bg-white border-violet-500 text-violet-700 shadow-md transform -translate-y-1";
                else if (isAnswering && isActiveBox) boxStyle = "bg-violet-50/50 border-violet-400 shadow-inner ring-4 ring-violet-200/50";
                ...
                return (
                  <div key={index} className={`relative w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center rounded-xl border-2 text-2xl sm:text-3xl font-bold uppercase transition-all duration-300 ${boxStyle}`}>
                    {displayChar}
                    {isAnswering && isActiveBox && <div className="absolute w-5 h-1 bg-violet-400 bottom-2 rounded-full animate-pulse"></div>}
                  </div>
                );
              });
            })()}
          ...
          {!isAnswering ? (
            <div className="flex flex-col sm:flex-row items-center justify-center w-full gap-4 sm:gap-6 mt-4">
              <button 
                ...
                className="w-full sm:w-auto flex flex-col items-center justify-center group px-8 py-3.5 rounded-xl border-2 border-violet-200 bg-violet-50 text-violet-600 hover:bg-violet-100 hover:border-violet-300 transition-all active:scale-95 disabled:opacity-50"
              >
              ...
              <button 
                ...
                className="w-full sm:w-auto flex flex-col items-center justify-center px-10 py-3.5 rounded-xl bg-violet-500 text-white shadow-md shadow-violet-200 hover:bg-violet-600 hover:shadow-lg hover:shadow-violet-300 transition-all active:scale-95 disabled:opacity-50"
              >
                <span className="font-bold text-lg">Cevapla</span>
                <span className="text-xs font-medium text-violet-100 mt-0.5">Tahmini Gir</span>
              </button>
            </div>
          ) : (
            <div className="w-full flex flex-col items-center mt-4 relative">
              ...
              <form onSubmit={handleSubmitAnswer} className="w-full flex flex-col items-center gap-3 relative">
                ...
                <button type="submit" className="px-10 py-3 bg-violet-600/90 text-white font-bold rounded-xl shadow-md hover:bg-violet-700 transition-all active:scale-95 text-lg">Gönder</button>
              </form>
            </div>
          )}
        </div>
